...
type MockSection = "intro" | Skill | "result";
...
export default function Home() {
  ...
  const recommendation = useMemo(() => getAdaptiveRecommendation(profile), [profile]);
  const bandGap = useMemo(() => getBandGap(profile), [profile]);

  function launchPractice(module?: Skill, mode?: string) {
  ...
  return (
    <div>
      ...
      <div className="mt-5 space-y-4">
        {session.items.map((item, index) => (
          <PracticeQuestion key={item.id} item={item} index={index} value={answers[item.id] ?? ""} onAnswer={onAnswer} />
        ))}
      </div>
